    ...
    def connect(self):
        ...
        try:
            if os.path.isdir(dir):
                self.conn = _s.connect(
                    "%s/pymodoro.db" % dir, detect_types=_s.PARSE_DECLTYPES)
            else:
                try:
                    os.mkdir(dir)
                    self.conn = _s.connect(
                        "%s/pymodoro.db" % dir,
                        detect_types=_s.PARSE_DECLTYPES)
                except:
                    exit('Erro ao criar diretorio')
            self.cursor = self.conn.cursor()
            self.cursor.execute(
                "CREATE TABLE IF NOT EXISTS tarefas(id INTEGER PRIMARY KEY, "
                "concluido int, tarefa text, pomodoros int, data timestamp )")
            return True
        except:
            return None
